	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				txset := types.NewTransactionsByPriceAndNonce(w.current.signer, txs, nil, false)
...
func (w *worker) commitTransaction(tx *types.Transaction, coinbase common.Address) ([]*types.Log, error) {
	...
	if !cfg.MSRAVMSettings.CmpReuse && cfg.MSRAVMSettings.GroundRecord {
		...
		receipt, err = w.chain.Cmpreuse.PreplayTransaction(w.chainConfig, w.chain, nil, &groundGasPool,
			groundStatedb, w.current.header, tx, &groundGasUsed, *cfg, 0, nil, 2,
			true, false)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if len(localTxs) > 0 {
		txs := types.NewTransactionsByPriceAndNonce(w.current.signer, localTxs, nil, false)
		if w.commitTransactions(txs, w.coinbase, interrupt) {
			return
		}
	}
	if len(remoteTxs) > 0 {
		txs := types.NewTransactionsByPriceAndNonce(w.current.signer, remoteTxs, nil, false)
		if w.commitTransactions(txs, w.coinbase, interrupt) {
			return
		}
	}
